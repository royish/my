...
$col_blgray =       $col_black . $col_brighten;



#useable colors
#
# black blgray red blred green blgreen brown blue blblue purple blpurple cyan blbyan ltgray
#
# usable effects
# underline blink rev_bg
...
$col_make =         $col_cyan;
$col_filename =     $col_black;
$col_linenum =      $col_cyan;
$col_trace =        $col_brown;
$col_warning =      $col_red;
$tag_error =        "Error: ";
$col_error =        $tag_error . $col_red . $col_brighten;
$error_highlight =  $col_black . $col_brighten;
...
while (<>)
{
	...
	elsif ($thisline =~ s/^[ \t]*(([^ \t]*)?(g?cc|(g|c)\\+\\+|ld|rg_gcc).*)$/$col_black$1$col_norm/) 
	{
		$in = 'gcc';
	}
	#@@@ sharong
