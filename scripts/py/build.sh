...
pybuild_add() {
	...
	export PREFIX="/opt/doxygen-1.7.6.1"
...
pybuild_doxygen() {
	dl_if_none doxygen-1.7.6.1.src.tar.gz http://sourceforge.net/projects/doxygen/files/rel-1.7.6.1/doxygen-1.7.6.1.src.tar.gz/download
	tar -C $BUILDDIR -zxf $SRCDIR/doxygen*
	cd $BUILDDIR/doxygen*
	pybuild_add
	./configure --prefix $PREFIX 
	make
...
pybuild_python() {
	...
	tar -C $BUILDDIR -zxf $SRCDIR/Python*
	cd $BUILDDIR/Python*
	pybuild_add
	pybuild_add_prepython
	./configure --enable-shared --prefix=$PREFIX --enable-ipv6  --with-dbmliborder=gdbm:bdb --with-threads
	make
	make install
	pybuild_remove
	build_cleanup
